function [height,state,o]=jumpMuscle(istate,istim,tStimStart,P,varargin)
% function [height,state,o]=jumpMuscle(tstart,istate,istim,P,varargin)
...
tStimStart=tStimStart-min(tStimStart)+0.01; %keep things positive in tstart.
istim = istim(:)';
...
tStimStart = tStimStart(:)'; %all of our muscle parameters are 1x6 vectors.
P.tstart = tStimStart;
P.sim.g=-9.81;
P.sim.air = 0;
...
[cmx,cmy,cmxp,cmyp,~,~]=xyCOM(x,y,xp,yp,xdp,ydp,l,d,mass);
%% simulate.
...
%%%%%% events
P.t_stim_sorted = sort(tStimStart);
P.i_mode = 1;
...
[cmx,cmy,cmxp,cmyp,~,~]=xyCOM(x,y,xp,yp,zeros(size(xp)),zeros(size(yp)),l,d,mass(:));
